...
def compile_cpp_code(configs, experiment_dir):
    ...
    # Check if we should use C++ compilation instead of Rust
    if configs.get("language", "rust").lower() == "cpp":
        compile_command = configs.get("compile-command", "cmake --build build --config Release")
    else:
    ...
    try:
        print("Compiling code with", compile_command)
        ...
        if compile_process.returncode != 0:
            ...
            if configs.get("settings", {}).get("ignore_compilation_errors", False):
                print("Ignoring compilation failure as configured.")
            else:
                sys.exit(1)
        ...
        print()
        print(colored(f"ERROR: Problems during compilation:", "red"), e)
        if configs.get("settings", {}).get("ignore_compilation_errors", False):
            print("Ignoring compilation failure as configured.")
        else:
            sys.exit(1)
...
def build_index(configs, experiment_dir):
    ...
    build_command = configs.get("build-command", "./build/bin/build_inverted_index")
    ...
    if configs.get("language", "rust").lower() == "cpp":
        ...
        # Add clustering algorithm if it exists
        if 'clustering-algorithm' in configs['indexing_parameters']:
            command_and_params.append(f"--clustering-algorithm {configs['indexing_parameters']['clustering-algorithm']}")
        
        # Add kmeans-doc-cut if it exists
        if 'kmeans-doc-cut' in configs['indexing_parameters']:
            command_and_params.append(f"--kmeans-doc-cut {configs['indexing_parameters']['kmeans-doc-cut']}")
        ...
        command_and_params = [
            build_command,
            f"--input-file {input_file}",
            f"--output-file {output_file}.index.seismic",
            f"--n-postings {configs['indexing_parameters']['n-postings']}",
            f"--summary-energy {configs['indexing_parameters']['summary-energy']}",
            f"--centroid-fraction {configs['indexing_parameters']['centroid-fraction']}",
    ...
    if ("NUMA" in configs['settings']):
        command = configs['settings']['NUMA'] + " " + command
    ...
    try:
        ...
        # Create a file to store the build output
        build_output_file = os.path.join(experiment_dir, "build.output")
        with open(build_output_file, "w") as build_output:
            for line in iter(build_process.stdout.readline, b''):
                decoded_line = line.decode()
                print(decoded_line, end='')
                build_output.write(decoded_line)
        ...
        if build_process.returncode != 0:
            print(colored("ERROR: Index building failed.", "red"))
            if configs.get("settings", {}).get("ignore_build_errors", False):
                print("Ignoring build failure as configured.")
                return building_time
            else:
                sys.exit(1)
        
        print(f"Index built successfully in {building_time:.2f} seconds.")
        return building_time
        
    except Exception as e:
        print(colored(f"ERROR: Problems during index building: {e}", "red"))
        if configs.get("settings", {}).get("ignore_build_errors", False):
            print("Ignoring build failure as configured.")
            return 0
        else:
...
def query_execution(configs, query_config, experiment_dir, subsection_name):
    ...
    # Get query command based on language
    if configs.get("language", "rust").lower() == "cpp":
        query_command = configs.get("query-command", "./build/bin/perf_inverted_index")
        # For C++ bindings, don't add .index.seismic extension
        index_path = index_file
    else:
        query_command = configs.get("query-command", "./target/release/perf_inverted_index")
        # For Rust bindings, add .index.seismic extension
        index_path = f"{index_file}.index.seismic"
    ...
    if query_process.returncode != 0:
        ...
        if configs.get("settings", {}).get("ignore_query_errors", False):
            print("Ignoring query failure as configured.")
            return query_time, 0, 0, memory_usage
        else:
            sys.exit(1)
...
def get_machine_info(configs, experiment_folder):
    ...
    if system == "Linux":
        # Linux-specific CPU frequency information
        machine_info.write(f"\n---------------------\n")
        ...
        cpus_with_performance_governor = 0
        for line in iter(governor.stdout.readline, b''):
            try:
                cpus_with_performance_governor = int(line.decode().strip())
            except ValueError:
                cpus_with_performance_governor = 0
                machine_info.write(f"Error getting CPU governor information\n")
        ...
        if (num_cpus != cpus_with_performance_governor):
            print()
            print(colored("WARNING: Your CPU may not be set to performance mode.", "yellow"))
            print()
        ...
        machine_info.write(f"\n-----------------\n")
        machine_info.write(f"macOS CPU information\n")
        machine_info.write(f"-----------------\n")
        
        # Get macOS CPU info using sysctl
        command_cpu = 'sysctl -a | grep -E "hw.model|hw.machine|hw.ncpu|hw.physicalcpu|hw.logicalcpu|hw.cpufrequency|machdep.cpu"'
        cpu = subprocess.Popen(command_cpu, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        cpu.wait()
...
def run_experiment(config_data):
   ...
   #print(config_data)
    ...
    # Store the output of the Git repository
    ...
    metric = config_data['settings'].get('metric', 'recall@10')
