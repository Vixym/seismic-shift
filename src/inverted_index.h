 ...
 */
 ...
 public:
         ...
         bool first_sorted) const 
     {
         ...
         if (knn_ && n_knn > 0) {
             knn_->refine(query, heap, visited, forward_index_, n_knn);
         }
     ...
     static InvertedIndex<T> build(const SparseDataset<T>& dataset, const Configuration& config) { 
         ...
         for (size_t doc_id = 0; doc_id < dataset.len(); doc_id += chunk_size) {
             ...
             if (pruning.get_type() == PruningStrategy::Type::FixedSize) {
                 this->fixed_pruning(inverted_pairs, pruning.get_n_postings());
             } else if (pruning.get_type() == PruningStrategy::Type::GlobalThreshold) {
                 this->global_threshold_pruning(inverted_pairs, pruning.get_n_postings());
             }
         ...
         if (pruning.get_type() == PruningStrategy::Type::GlobalThreshold) {
             ...
             this->fixed_pruning(inverted_pairs, max_postings);
         }
         ...
         if (config.get_knn().get_nknn() == 0 && !config.get_knn().get_knn_path().has_value()) {
             return index;
         }
         
         time_start = std::chrono::high_resolution_clock::now();
         auto knn_config = config.get_knn();
    ...
    public:
        ...
        void search(
            ...
            for (size_t i = 0; i < dots.size(); ++i) {
                indexed_dots.emplace_back(i, dots[i]);
            }
        ...
        static PostingList build(
            ...
            return PostingList(
                ...
                QuantizedSummary::from(SparseDataset<T>::from(summaries_dataset))
            );
        }
    ...
    private:
        ...
        static std::vector<size_t> blocking_with_random_kmeans(
            ...
            switch (clustering_algorithm.get_type()) {
                case ClusteringAlgorithm::Type::RandomKmeans:
                    clustering_results = utils::do_random_kmeans_on_docids(
                        posting_list,
                        n_centroids,
                        dataset,
                        min_cluster_size
        ...
        static std::pair<std::vector<uint16_t>, std::vector<T>> fixed_size_summary(
            ...
            for (size_t doc_id : block) {
                ...
                for (const auto& [component_id, value] : dataset.iter_vector(doc_id)) {
 ...
 */
 class Knn {
    ...
    public:
        static const size_t KNN_QUERY_CUT = 10;
        static const float KNN_HEAP_FACTOR = 0.7f;
        ...
        static Knn new_from_serialized(const std::string& path, std::optional<size_t> limit = std::nullopt) {
            ...
            std::cout << "Number of neighbors in the file: " << d << std::endl;
    
            // If limit is specified and less than d, create a new Knn with fewer neighbors
            size_t nknn = limit.value_or(d);
    
            if (nknn > d) {
                throw std::invalid_argument("The number of neighbors to include for each vector of the dataset can't be greater than the number of neighbours in the precomputed knn file.");
            }
    
            if (nknn == d) {
                return Knn(n_vecs, d, std::move(neighbours), nbits);
            } else {
                std::cout << "We only take " << nknn << " neighbors per element!" << std::endl;
                
                // Create a new bit vector with fewer neighbors per vector
                BitVectorMut new_neighbours;
                new_neighbours.reserve(n_vecs * nbits * nknn);
                
                for (size_t id = 0; id < n_vecs; ++id) {
                    for (size_t i = 0; i < nknn; ++i) {
                        size_t bit_offset = id * d * nbits + i * nbits;
                        uint64_t neighbor = neighbours.get_bits(bit_offset, nbits);
                        new_neighbours.append_bits(neighbor, nbits);
                    }
                }
                
                return Knn(n_vecs, nknn, BitVector(std::move(new_neighbours)), nbits);
            }
        }
    
        // Serialize the KNN graph to a file
        void serialize(const std::string& output_file) const {
            std::string path = output_file + ".knn.seismic";
        ...
        static std::pair<BitVector, size_t> compress_into_bitvector(
            const std::vector<uint64_t>& data, size_t n_vecs, size_t d) {
            ...
            BitVectorMut neighbours;
            neighbours.reserve(data.size() * nbits);
            ...
            return {BitVector(std::move(neighbours)), nbits};
        }
        ...
        void refine(const std::vector<float>& query,
            ...
            for (const auto& [_distance, offset] : neighbours_results) {
                ...
                for (size_t i = 0; i < n_knn; ++i) {
                    size_t bit_offset = id * d * nbits + i * nbits;
                    uint64_t neighbour = neighbours.get_bits(bit_offset, nbits);
