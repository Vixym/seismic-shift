...
namespace seismic
{
    ...
    template <typename T>
    size_t InvertedIndex<T>::print_space_usage_byte() const
    {
        size_t total_size = space_usage_byte();
        std::cout << "Total space usage: " << total_size << " bytes" << std::endl;
        return total_size;
    }
    ...
    template size_t InvertedIndex<float>::print_space_usage_byte() const;
    template size_t InvertedIndex<double>::print_space_usage_byte() const;
    template size_t InvertedIndex<Float16>::print_space_usage_byte() const;
